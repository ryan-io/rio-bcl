...
namespace Procedural {
	...
	public class ProceduralTileSolver : Singleton<ProceduralTileSolver, IProceduralTileSolver>,
		...
		public UniTask Enable(CancellationToken token) {
			var mapConfig = MonoModel.ProceduralMapSolver.MonoModel.ProceduralProceduralMapConfig;
			var tileSolverModel = new TileSolverModel(
				MonoModel, Model, mapConfig.MapWidth, mapConfig.MapHeight, mapConfig.CellSize);
		...
		public UniTask Begin(CancellationToken token) {
			var mapConfig = MonoModel.ProceduralMapSolver.MonoModel.ProceduralProceduralMapConfig;
			var grid      = MonoModel.TileMapGameObjects.GridObject;
			...
			grid.transform.position = new Vector3(
				-mapConfig.MapWidth  / 2f + mapConfig.MapBorderSize / 2f,
				-mapConfig.MapHeight / 2f + mapConfig.MapBorderSize / 2f,
				0f);
		...
		void IValidate.ValidateShouldQuit() {
			...
			var statements = new HashSet<Func<bool>> {
				...
				() => MonoModel.TileMapGameObjects.GridObject       == null,
