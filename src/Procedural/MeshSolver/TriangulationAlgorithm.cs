...
namespace Procedural {
	public class TriangulationAlgorithm {
		...
		public void TriangulateSquare(
			Square square, HashSet<int> checkedVertices, Dictionary<int, List<MeshTriangle>> triangleTracker) {
		...
		void MeshFromPoints(IDictionary<int, List<MeshTriangle>> triangleTracker, params Node[] points) {
		...
		void CreateTriangle(Node a, Node b, Node c, IDictionary<int, List<MeshTriangle>> triangleTracker) {
			GetWalkableTriangles.Add(a.VertexIndex);
			GetWalkableTriangles.Add(b.VertexIndex);
			GetWalkableTriangles.Add(c.VertexIndex);

			var triangle = new MeshTriangle(a.VertexIndex, b.VertexIndex, c.VertexIndex);
		...
		void TrackTriangle(int vertexKey, MeshTriangle triangle, IDictionary<int, List<MeshTriangle>> triangleTracker) {
			if (triangleTracker.ContainsKey(vertexKey))
				triangleTracker[vertexKey].Add(triangle);

			else
				triangleTracker.Add(vertexKey, new List<MeshTriangle> { triangle });
		}
	}
}
