...
namespace UnityBCL {
	public static class Utility {
		...
		public static readonly Camera? CameraMain   = Camera.main;
		static readonly        Vector3 Vector3Zero  = Vector3.zero;
		static readonly        Vector3 Vector3One   = Vector3.one;
		static readonly        Vector3 Vector3YDown = new(0, -1);

		static Transform? _cachedCanvasTransform;
		static int        _sequencialColorIndex = -1;


		static readonly Color[] SequencialColors = {
		...
		public static Vector3 MousePos2D => GetMouseWorldPosition();

		public static bool FiftyFiftyChance() => Random.Next(2) == 1;

		public static float GetFloatRand(float minInclusive, float maxInclusive, double scalar = 1) {
			var value = scalar * (Random.NextDouble() * (maxInclusive - minInclusive) + minInclusive);
			return (float)value;
		}

		public static int GetIntRan(int minInclusive, int maxInclusive, int scalar) {
			var value = scalar * Random.Next(minInclusive, maxInclusive);
			return value;
		}

		public static TextMeshPro CreateWorldTextTMP(string text, Transform parent, int fontSize,
			Color color, TextAlignmentOptions textAlignment,
			Vector3 offsetPosition, Sprite? sprite = null) {
			...
			tmp.rectTransform.localPosition = offsetPosition;
		...
		public static Transform? GetCanvasTransform() {
			if (_cachedCanvasTransform == null) {
				...
				if (canvas != null) _cachedCanvasTransform = canvas.transform;
			}
		...
		// 		return false;
		// 	
		// 	log.Error("The provided object was null. No other context provided.", "IsNullObject");
		// 	return true;
		// }

		public static bool IsNullReference<T>(T? obj, string id) where T : class {
			if (obj != null)
				return false;
		...
		public static ButtonSprite CreateWorldSpriteButton(string name, Sprite sprite, Vector3 localPosition,
			Vector3 localScale, int sortingOrder, Color color)
		...
		public static ButtonSprite CreateWorldSpriteButton(Transform? parent, string name, Sprite sprite,
			...
			var buttonSprite = gameObject.AddComponent<ButtonSprite>();
			return buttonSprite;
		}
		...
		public static Text DrawTextUI(string textString, Transform? parent, Vector2 anchoredPosition,
			int fontSize,
			Font font) {
		...
		public static float Parse_Float(string txt, float _default) {
			if (!float.TryParse(txt, out var f)) f = _default;
		...
		public static int Parse_Int(string txt, int _default) {
			if (!int.TryParse(txt, out var i)) i = _default;
		...
		public static Vector3 GetMouseWorldPosition() {
			if (CameraMain != null) {
				var vec = GetMouseWorldPositionWithZ(Input.mousePosition, CameraMain);
				vec.z = 0f;
				return vec;
			}
		...
		public static Vector3 GetMouseWorldPositionWithZ() {
			if (CameraMain != null) return GetMouseWorldPositionWithZ(Input.mousePosition, CameraMain);
			return Vector3.positiveInfinity;
		}
		...
		public static Color GetSequencialColor() {
			_sequencialColorIndex = (_sequencialColorIndex + 1) % SequencialColors.Length;
			return SequencialColors[_sequencialColorIndex];
		}
		...
		public static Vector3 ApplyRotationToVectorXZ(Vector3 vec, float angle) => Quaternion.Euler(0, angle, 0) * vec;
		...
		public static string SaveDictionaryJson<TKey, TValue>(Dictionary<TKey, TValue> dictionary) {
			...
			var saveJson = JsonUtility.ToJson(jsonDictionary);
			return saveJson;
		}
		...
		public static Dictionary<TKey, TValue> LoadDictionaryJson<TKey, TValue>(string saveJson) {
			var jsonDictionary = JsonUtility.FromJson<JsonDictionary>(saveJson);
			var ret            = new Dictionary<TKey, TValue>();
			for (var i = 0; i < jsonDictionary.keyList.Count; i++) {
				var key   = JsonUtility.FromJson<TKey>(jsonDictionary.keyList[i]);
				var value = JsonUtility.FromJson<TValue>(jsonDictionary.valueList[i]);
				ret[key] = value;
			}
		...
		public static void SetAllChildrenLayer(Transform parent, int layer) {
			...
			foreach (Transform trans in parent) SetAllChildrenLayer(trans, layer);
		}


		// Returns a random script that can be used to id
		public static string GetIdString() {
			var alphabet                    = "0123456789abcdefghijklmnopqrstuvxywz";
			var ret                         = "";
			for (var i = 0; i < 8; i++) ret += alphabet[UnityEngine.Random.Range(0, alphabet.Length)];
		...
		public static string GetIdStringLong(int chars) {
			var alphabet                        = "0123456789abcdefghijklmnopqrstuvxywzABCDEFGHIJKLMNOPQRSTUVXYWZ";
			var ret                             = "";
			for (var i = 0; i < chars; i++) ret += alphabet[UnityEngine.Random.Range(0, alphabet.Length)];
		...
		public static string GetRandomName(bool withSurname = false) {
			var firstNameList = new List<string> {
				"Gabe", "Cliff", "Tim", "Ron", "Jon", "John", "Mike", "Seth",
				"Alex", "Steve", "Chris", "Will", "Bill",
				"James", "Jim",
				"Ahmed", "Omar", "Peter", "Pierre", "George", "Lewis", "Lewie",
				"Adam", "William", "Ali", "Eddie", "Ed",
				"Dick", "Robert", "Bob", "Rob",
				"Neil", "Tyson", "Carl", "Chris", "Christopher", "Jensen",
				"Gordon", "Morgan", "Richard", "Wen", "Wei",
				"Luke", "Lucas", "Noah", "Ivan", "Yusuf",
				"Ezio", "Connor", "Milan", "Nathan", "Victor", "Harry", "Ben",
				"Charles", "Charlie", "Jack", "Leo",
				"Leonardo", "Dylan", "Steven", "Jeff",
				"Alex", "Mark", "Leon", "Oliver", "Danny", "Liam", "Joe", "Tom",
				"Thomas", "Bruce", "Clark", "Tyler",
				"Jared", "Brad", "Jason"
			};

			if (!withSurname) return firstNameList[UnityEngine.Random.Range(0, firstNameList.Count)];

			var alphabet = "ABCDEFGHIJKLMNOPQRSTUVXYWZ";
		...
		public static string GetRandomCityName() {
			var cityNameList = new List<string> {
				"Alabama", "New York", "Old York", "Bangkok", "Lisbon", "Vee",
				"Agen", "Agon", "Ardok", "Arbok",
				"Kobra", "House", "Noun", "Hayar", "Salma", "Chancellor", "Dascomb",
				"Payn", "Inglo", "Lorr", "Ringu",
				"Brot", "Mount Loom", "Kip", "Chicago", "Madrid", "London", "Gam",
				"Greenvile", "Franklin", "Clinton", "Springfield", "Salem",
				"Fairview", "Fairfax", "Washington",
				"Madison",
				"Georgetown", "Arlington", "Marion", "Oxford", "Harvard", "Valley",
				"Ashland", "Burlington",
				"Manchester", "Clayton",
				"Milton", "Auburn", "Dayton", "Lexington", "Milford", "Riverside",
				"Cleveland", "Dover", "Hudson",
				"Kingston", "Mount Vernon",
				"Newport", "Oakland", "Centerville", "Winchester", "Rotary",
				"Bailey", "Saint Mary", "Three Waters",
				"Veritas", "Chaos", "Center",
				"Millbury", "Stockland", "Deerstead Hills", "Plaintown",
				"Fairchester", "Milaire View", "Bradton",
				"Glenfield", "Kirkmore",
				"Fortdell", "Sharonford", "Inglewood", "Englecamp", "Harrisvania",
				"Bosstead", "Brookopolis",
				"Metropolis", "Colewood", "Willowbury",
				"Hearthdale", "Weelworth", "Donnelsfield", "Greenline", "Greenwich",
				"Clarkswich", "Bridgeworth",
				"Normont",
				"Lynchbrook", "Ashbridge", "Garfort", "Wolfpain", "Waterstead",
				"Glenburgh", "Fortcroft", "Kingsbank",
				"Adamstead", "Mistead",
				"Old Crossing", "Crossing", "New Agon", "New Agen", "Old Agon",
				"New Valley", "Old Valley",
				"New Kingsbank", "Old Kingsbank",
				"New Dover", "Old Dover", "New Burlington", "Shawshank",
				"Old Shawshank", "New Shawshank",
				"New Bradton", "Old Bradton", "New Metropolis", "Old Clayton",
				"New Clayton"
			};
		...
		public static bool IsPositionInsideFov(Vector3 pos, Vector3 aimDir, Vector3 posTarget, float fov) {
			var aimAngle                                = GetAngleFromVector180(aimDir);
			var angle                                   = GetAngleFromVector180(posTarget - pos);
			var angleDifference                         = angle - aimAngle;
			if (angleDifference > 180) angleDifference  -= 360;
			if (angleDifference < -180) angleDifference += 360;
			if (!(angleDifference < fov / 2f && angleDifference > -fov / 2f)) // Not inside fov
				return false;
			return true;
		}
		...
		public static void GetTimeCharacterStrings(float time, out string h0, out string h1, out string m0,
			out string m1, out string s0, out string s1, out string ms0,
			out string ms1, out string ms2) {
			var s = Mathf.FloorToInt(time);
			var m = Mathf.FloorToInt(s / 60f);
			var h = Mathf.FloorToInt(s / 60f / 60f);
			s = s - m * 60;
			m = m - h * 60;
			var ms = (int)((time - Mathf.FloorToInt(time)) * 1000);
			...
			if (ms < 10) {
				...
				if (ms < 100) {
					...
					// >= 100
					var _i_ms0 = Mathf.FloorToInt(ms / 100f);
					var _i_ms1 = Mathf.FloorToInt(ms / 10f) - _i_ms0 * 10;
					var _i_ms2 = ms                         - _i_ms1 * 10 - _i_ms0 * 100;
					ms0 = "" + _i_ms0;
					ms1 = "" + _i_ms1;
					ms2 = "" + _i_ms2;
				}
		...
		public static float GetTimeMilliseconds(float startTime) => (Time.realtimeSinceStartup - startTime) * 1000f;
		...
		public static List<Vector3> GetPositionListAround(Vector3 position, float distance, int positionCount) {
			...
			for (var i = 0; i < positionCount; i++) {
				...
				var dir   = ApplyRotationToVector(new Vector3(0, 1), angle);
				var pos   = position + dir * distance;
				ret.Add(pos);
			}
		...
		public static List<Vector3> GetPositionListAround(Vector3 position, float[] ringDistance,
			int[] ringPositionCount) {
			var ret = new List<Vector3>();
			for (var ring = 0; ring < ringPositionCount.Length; ring++) {
				var ringPositionList =
					GetPositionListAround(position, ringDistance[ring], ringPositionCount[ring]);
		...
		public static List<Vector3> GetPositionListAround(Vector3 position, float distance, int positionCount,
			Vector3 direction, int angleStart, int angleIncrease) {
			var ret = new List<Vector3>();
			for (var i = 0; i < positionCount; i++) {
				var angle = angleStart + angleIncrease * i;
				var dir   = ApplyRotationToVector(direction, angle);
				var pos   = position + dir * distance;
				ret.Add(pos);
			}
		...
		public static List<Vector3> GetPositionListAlongDirection(Vector3 position, Vector3 direction,
			float distancePerPosition, int positionCount) {
			var ret = new List<Vector3>();
			for (var i = 0; i < positionCount; i++) {
				var pos = position + direction * (distancePerPosition * i);
				ret.Add(pos);
			}
		...
		public static List<Vector3> GetPositionListAlongAxis(Vector3 positionStart, Vector3 positionEnd,
			int positionCount) {
			var direction           = (positionEnd - positionStart).normalized;
			var distancePerPosition = (positionEnd - positionStart).magnitude / positionCount;
			return GetPositionListAlongDirection(positionStart + direction * (distancePerPosition / 2f), direction,
				distancePerPosition,                                                                     positionCount);
		}

		public static List<Vector3>
			GetPositionListWithinRect(Vector3 lowerLeft, Vector3 upperRight, int positionCount) {
			var     ret                = new List<Vector3>();
			var     width              = upperRight.x - lowerLeft.x;
			var     height             = upperRight.y - lowerLeft.y;
			var     area               = width * height;
			var     areaPerPosition    = area  / positionCount;
			var     positionSquareSize = Mathf.Sqrt(areaPerPosition);
			Vector3 rowLeft, rowRight;
			rowLeft  = new Vector3(lowerLeft.x,  lowerLeft.y);
			rowRight = new Vector3(upperRight.x, lowerLeft.y);
			var rowsTotal = Mathf.RoundToInt(height / positionSquareSize);
			var increaseY = height / rowsTotal;
			rowLeft.y  += increaseY / 2f;
			rowRight.y += increaseY / 2f;
			var positionsPerRow = Mathf.RoundToInt(width / positionSquareSize);
			...
			var missingPositions = positionCount - ret.Count;
			var angleDir         = (upperRight - lowerLeft) / missingPositions;
			for (var i = 0; i < missingPositions; i++) ret.Add(lowerLeft + angleDir / 2f + angleDir * i);

			while (ret.Count > positionCount) ret.RemoveAt(UnityEngine.Random.Range(0, ret.Count));
		...
		public static List<Vector2Int> GetPosXYListOblong(int width, int dropXamount, int increaseDropXamount,
			Vector3 dir) {
			var list = GetPosXYListOblong(width, dropXamount, increaseDropXamount);
			list = RotatePosXYList(list, GetAngleFromVector(dir));
			return list;
		}

		public static List<Vector2Int> GetPosXYListOblong(int width, int dropXamount, int increaseDropXamount) {
			var triangle = GetPosXYListTriangle(width, dropXamount, increaseDropXamount);
			var list     = new List<Vector2Int>(triangle);
			foreach (var posXY in triangle) {
				if (posXY.y == 0) continue;
				list.Add(new Vector2Int(posXY.x, -posXY.y));
			}

			foreach (var posXY in new List<Vector2Int>(list)) {
		...
		public static List<Vector2Int> GetPosXYListTriangle(int width, int dropXamount, int increaseDropXamount) {
			var list = new List<Vector2Int>();
			for (var i = 0; i > -999; i--) {
				for (var j = 0; j < width; j++) list.Add(new Vector2Int(j, i));
		...
		public static List<Vector2Int> RotatePosXYList(List<Vector2Int> list, int angle) {
			...
			for (var i = 0; i < list.Count; i++) {
				var posXY = list[i];
				var vec   = ApplyRotationToVector(new Vector3(posXY.x, posXY.y), angle);
		...
		public static Transform CloneTransform(Transform transform, string? name = null) {
			var clone = Object.Instantiate(transform, transform.parent);
			clone.name = name ?? transform.name;

			return clone;
		}

		public static Transform CloneTransform(Transform transform, string name, Vector2 newAnchoredPosition,
			bool setActiveTrue = false) {
			var clone = CloneTransform(transform, name);
			clone.GetComponent<RectTransform>().anchoredPosition = newAnchoredPosition;
			if (setActiveTrue) clone.gameObject.SetActive(true);

			return clone;
		}

		public static Transform CloneTransform(Transform transform, Transform newParent, string? name = null) {
			var clone = Object.Instantiate(transform, newParent);
		...
		public static Transform CloneTransform(Transform transform, Transform newParent, string name,
			Vector2 newAnchoredPosition, bool setActiveTrue = false) {
			var clone = CloneTransform(transform, newParent, name);
			clone.GetComponent<RectTransform>().anchoredPosition = newAnchoredPosition;
			if (setActiveTrue) clone.gameObject.SetActive(true);
		...
		public static Transform CloneTransformWorld(Transform transform, Transform newParent, string name,
		...
		public static void ShuffleArray<T>(T[] arr, int iterations) {
			for (var i = 0; i < iterations; i++) {
				var rnd = UnityEngine.Random.Range(0, arr.Length);
				var tmp = arr[rnd];
				arr[rnd] = arr[0];
				arr[0]   = tmp;
			}
		...
		public static void ShuffleArray<T>(T[] arr, int iterations, Random random) {
			for (var i = 0; i < iterations; i++) {
		...
		public static void DebugDrawCircle(Vector3 center, float radius, Color color, float duration, int divisions) {
			for (var i = 0; i <= divisions; i++) {
				var vec1 = center + ApplyRotationToVector(new Vector3(0, 1) * radius, 360f / divisions * i);
				var vec2 = center +
				           ApplyRotationToVector(new Vector3(0, 1) * radius, 360f / divisions * (i + 1));
			...
			switch (c) {
				...
				case 'A':
					DebugDrawLines(position, color, size, duration, new[] {
						0.317f, 0.041f, 0.5f, 0.98f, 0.749f,
						0.062f, 0.625f, 0.501f, 0.408f, 0.507f
					});
					break;
				case 'B':
					DebugDrawLines(position, color, size, duration, new[] {
						0.289f, 0.069f, 0.274f, 0.937f, 0.609f,
						0.937f, 0.801f, 0.879f, 0.829f, 0.708f,
						0.756f, 0.538f,
						0.655f, 0.492f, 0.442f, 0.495f, 0.271f,
						0.495f, 0.567f, 0.474f, 0.676f, 0.465f,
						0.722f, 0.385f,
						0.719f, 0.181f, 0.664f, 0.087f, 0.527f,
						0.053f, 0.396f, 0.05f, 0.271f, 0.078f
					});
					break;
				case 'C':
					DebugDrawLines(position, color, size, duration, new[] {
						0.695f, 0.946f, 0.561f, 0.949f, 0.426f,
						0.937f, 0.317f, 0.867f, 0.265f, 0.733f,
						0.262f, 0.553f,
						0.292f, 0.27f, 0.323f, 0.172f, 0.417f,
						0.12f, 0.512f, 0.096f, 0.637f, 0.093f,
						0.743f, 0.117f
					});
					break;
				case 'D':
					DebugDrawLines(position, color, size, duration, new[] {
						0.314f, 0.909f, 0.329f, 0.096f, 0.53f,
						0.123f, 0.594f, 0.197f, 0.673f, 0.334f,
						0.716f, 0.498f,
						0.692f, 0.666f, 0.609f, 0.806f, 0.457f,
						0.891f, 0.323f, 0.919f
					});
					break;
				case 'E':
					DebugDrawLines(position, color, size, duration, new[] {
						0.344f, 0.919f, 0.363f, 0.078f, 0.713f,
						0.096f, 0.359f, 0.096f, 0.347f, 0.48f,
						0.53f, 0.492f,
						0.356f, 0.489f, 0.338f, 0.913f, 0.625f,
						0.919f
					});
					break;
				case 'F':
					DebugDrawLines(position, color, size, duration, new[] {
						0.682f, 0.916f, 0.329f, 0.909f, 0.341f,
						0.66f, 0.503f, 0.669f, 0.341f, 0.669f,
						0.317f, 0.087f
					});
					break;
				case 'G':
					DebugDrawLines(position, color, size, duration, new[] {
						0.618f, 0.867f, 0.399f, 0.849f, 0.292f,
						0.654f, 0.241f, 0.404f, 0.253f, 0.178f,
						0.481f, 0.075f,
						0.612f, 0.078f, 0.725f, 0.169f, 0.728f,
						0.334f, 0.71f, 0.437f, 0.609f, 0.462f,
						0.463f, 0.462f
					});
					break;
				case 'H':
					DebugDrawLines(position, color, size, duration, new[] {
						0.277f, 0.876f, 0.305f, 0.133f, 0.295f,
						0.507f, 0.628f, 0.501f, 0.643f, 0.139f,
						0.637f, 0.873f
					});
					break;
				case 'I':
				...
				case 'J':
					DebugDrawLines(position, color, size, duration, new[] {
						0.628f, 0.882f, 0.679f, 0.242f, 0.603f,
						0.114f, 0.445f, 0.066f, 0.317f, 0.114f,
						0.262f, 0.209f,
						0.253f, 0.3f, 0.259f, 0.367f
					});
					break;
				case 'K':
					DebugDrawLines(position, color, size, duration, new[] {
						0.292f, 0.879f, 0.311f, 0.111f, 0.305f,
						0.498f, 0.594f, 0.876f, 0.305f, 0.516f,
						0.573f, 0.154f
					});
					break;
				case 'L':
					DebugDrawLines(position, color, size, duration, new[] {
						0.311f, 0.879f, 0.308f, 0.133f, 0.682f,
						0.148f
					});
					break;
				case 'M':
					DebugDrawLines(position, color, size, duration, new[] {
						0.262f, 0.12f, 0.265f, 0.909f, 0.509f,
						0.608f, 0.71f, 0.919f, 0.713f, 0.151f
					});
					break;
				case 'N':
					DebugDrawLines(position, color, size, duration, new[] {
						0.737f, 0.885f, 0.679f, 0.114f, 0.335f,
						0.845f, 0.353f, 0.175f
					});
					break;
				case 'O':
					DebugDrawLines(position, color, size, duration, new[] {
						0.609f, 0.906f, 0.396f, 0.894f, 0.271f,
						0.687f, 0.232f, 0.474f, 0.241f, 0.282f,
						0.356f, 0.142f,
						0.527f, 0.087f, 0.655f, 0.09f, 0.719f,
						0.181f, 0.737f, 0.379f, 0.737f, 0.638f,
						0.71f, 0.836f,
						0.628f, 0.919f, 0.582f, 0.919f
					});
					break;
				case 'P':
					DebugDrawLines(position, color, size, duration, new[] {
						0.314f, 0.129f, 0.311f, 0.873f, 0.658f,
						0.906f, 0.746f, 0.8f, 0.746f, 0.66f,
						0.673f, 0.544f,
						0.509f, 0.51f, 0.359f, 0.51f, 0.311f,
						0.516f
					});
					break;
				case 'Q':
					DebugDrawLines(position, color, size, duration, new[] {
						0.497f, 0.925f, 0.335f, 0.894f, 0.228f,
						0.681f, 0.213f, 0.379f, 0.25f, 0.145f,
						0.396f, 0.096f,
						0.573f, 0.105f, 0.631f, 0.166f, 0.542f,
						0.245f, 0.752f, 0.108f, 0.628f, 0.187f,
						0.685f, 0.261f,
						0.728f, 0.398f, 0.759f, 0.605f, 0.722f,
						0.794f, 0.64f, 0.916f, 0.475f, 0.946f
					});
					break;
				case 'R':
					DebugDrawLines(position, color, size, duration, new[] {
						0.347f, 0.142f, 0.332f, 0.9f, 0.667f,
						0.897f, 0.698f, 0.699f, 0.655f, 0.58f,
						0.521f, 0.553f,
						0.396f, 0.553f, 0.344f, 0.553f, 0.564f,
						0.37f, 0.655f, 0.206f, 0.71f, 0.169f
					});
					break;
				case 'S':
					DebugDrawLines(position, color, size, duration, new[] {
						0.695f, 0.842f, 0.576f, 0.882f, 0.439f,
						0.885f, 0.329f, 0.8f, 0.289f, 0.626f,
						0.317f, 0.489f,
						0.439f, 0.44f, 0.621f, 0.434f, 0.695f,
						0.358f, 0.713f, 0.224f, 0.646f, 0.111f,
						0.494f, 0.093f,
						0.338f, 0.105f, 0.289f, 0.151f
					});
					break;
				case 'T':
					DebugDrawLines(position, color, size, duration, new[] {
						0.497f, 0.172f, 0.5f, 0.864f, 0.286f,
						0.858f, 0.719f, 0.852f
					});
					break;
				case 'U':
					DebugDrawLines(position, color, size, duration, new[] {
						0.232f, 0.858f, 0.247f, 0.251f, 0.366f,
						0.105f, 0.466f, 0.078f, 0.615f, 0.084f,
						0.704f, 0.123f,
						0.746f, 0.276f, 0.74f, 0.559f, 0.737f,
						0.806f, 0.722f, 0.864f
					});
					break;
				case 'V':
					DebugDrawLines(position, color, size, duration, new[] {
						0.238f, 0.855f, 0.494f, 0.105f, 0.707f,
						0.855f
					});
					break;
				case 'X':
					DebugDrawLines(position, color, size, duration, new[] {
						0.783f, 0.852f, 0.256f, 0.133f, 0.503f,
						0.498f, 0.305f, 0.824f, 0.789f, 0.117f
					});
					break;
				case 'Y':
					DebugDrawLines(position, color, size, duration, new[] {
						0.299f, 0.842f, 0.497f, 0.529f, 0.646f,
						0.842f, 0.49f, 0.541f, 0.487f, 0.105f
					});
					break;
				case 'W':
					DebugDrawLines(position, color, size, duration, new[] {
						0.228f, 0.815f, 0.381f, 0.093f, 0.503f,
						0.434f, 0.615f, 0.151f, 0.722f, 0.818f
					});
					break;
				case 'Z':
					DebugDrawLines(position, color, size, duration, new[] {
						0.25f, 0.87f, 0.795f, 0.842f, 0.274f,
						0.133f, 0.716f, 0.142f
					});
					break;
				...
				case '0':
					DebugDrawLines(position, color, size, duration, new[] {
						0.536f, 0.891f, 0.509f, 0.891f, 0.42f,
						0.809f, 0.378f, 0.523f, 0.372f, 0.215f,
						0.448f, 0.087f,
						0.539f, 0.069f, 0.609f, 0.099f, 0.637f,
						0.242f, 0.646f, 0.416f, 0.646f, 0.608f,
						0.631f, 0.809f,
						0.554f, 0.888f, 0.527f, 0.894f
					});
					break;
				case '1':
					DebugDrawLines(position, color, size, duration, new[] {
						0.652f, 0.108f, 0.341f, 0.114f, 0.497f,
						0.12f, 0.497f, 0.855f, 0.378f, 0.623f
					});
					break;
				case '2':
					DebugDrawLines(position, color, size, duration, new[] {
						0.311f, 0.714f, 0.375f, 0.83f, 0.564f,
						0.894f, 0.722f, 0.839f, 0.765f, 0.681f,
						0.634f, 0.483f,
						0.5f, 0.331f, 0.366f, 0.245f, 0.299f,
						0.126f, 0.426f, 0.126f, 0.621f, 0.136f,
						0.679f, 0.136f,
						0.737f, 0.139f
					});
					break;
				case '3':
					DebugDrawLines(position, color, size, duration, new[] {
						0.289f, 0.855f, 0.454f, 0.876f, 0.606f,
						0.818f, 0.685f, 0.702f, 0.664f, 0.547f,
						0.564f, 0.459f,
						0.484f, 0.449f, 0.417f, 0.455f, 0.53f,
						0.434f, 0.655f, 0.355f, 0.664f, 0.233f,
						0.591f, 0.105f,
						0.466f, 0.075f, 0.335f, 0.084f, 0.259f,
						0.142f
					});
					break;
				case '4':
					DebugDrawLines(position, color, size, duration, new[] {
						0.353f, 0.836f, 0.262f, 0.349f, 0.579f,
						0.367f, 0.5f, 0.376f, 0.49f, 0.471f,
						0.509f, 0.069f
					});
					break;
				case '5':
					DebugDrawLines(position, color, size, duration, new[] {
						0.67f, 0.852f, 0.335f, 0.858f, 0.347f,
						0.596f, 0.582f, 0.602f, 0.698f, 0.513f,
						0.749f, 0.343f,
						0.719f, 0.187f, 0.561f, 0.133f, 0.363f,
						0.151f
					});
					break;
				case '6':
					DebugDrawLines(position, color, size, duration, new[] {
						0.567f, 0.888f, 0.442f, 0.782f, 0.35f,
						0.544f, 0.326f, 0.288f, 0.39f, 0.157f,
						0.615f, 0.142f,
						0.679f, 0.245f, 0.676f, 0.37f, 0.573f,
						0.48f, 0.454f, 0.48f, 0.378f, 0.41f,
						0.335f, 0.367f
					});
					break;
				case '7':
					DebugDrawLines(position, color, size, duration, new[] {
						0.286f, 0.852f, 0.731f, 0.864f, 0.417f,
						0.117f, 0.57f, 0.498f, 0.451f, 0.483f,
						0.688f, 0.501f
					});
					break;
				case '8':
					DebugDrawLines(position, color, size, duration, new[] {
						0.518f, 0.541f, 0.603f, 0.623f, 0.649f,
						0.748f, 0.612f, 0.858f, 0.497f, 0.888f,
						0.375f, 0.824f,
						0.341f, 0.708f, 0.381f, 0.611f, 0.494f,
						0.55f, 0.557f, 0.513f, 0.6f, 0.416f,
						0.631f, 0.312f,
						0.579f, 0.178f, 0.509f, 0.108f, 0.436f,
						0.102f, 0.335f, 0.181f, 0.308f, 0.279f,
						0.347f, 0.401f,
						0.423f, 0.486f, 0.497f, 0.547f
					});
					break;
				case '9':
					DebugDrawLines(position, color, size, duration, new[] {
						0.475f, 0.129f, 0.573f, 0.495f, 0.646f,
						0.824f, 0.509f, 0.97f, 0.28f, 0.94f,
						0.189f, 0.827f,
						0.262f, 0.708f, 0.396f, 0.69f, 0.564f,
						0.745f, 0.646f, 0.83f
					});
					break;
				...
				case '.':
					DebugDrawLines(position, color, size, duration, new[] {
						0.515f, 0.157f, 0.469f, 0.148f, 0.469f,
						0.117f, 0.515f, 0.123f, 0.503f, 0.169f
					});
					break;
				case ':':
					DebugDrawLines(position, color, size, duration, new[] {
					...
					DebugDrawLines(position, color, size, duration, new[] {
						0.515f, .5f + 0.157f, 0.469f,
						.5f         + 0.148f, 0.469f, .5f + 0.117f,
						0.515f, .5f + 0.123f, 0.503f,
						.5f         + 0.169f
					});
					break;
				case '-':
				...
				case '+':
					DebugDrawLines(position, color, size, duration, new[] {
						0.265f, 0.513f, 0.676f, 0.516f, 0.497f,
						0.529f, 0.49f, 0.699f, 0.497f, 0.27f
					});
					break;

				case '(':
					DebugDrawLines(position, color, size, duration, new[] {
						0.542f, 0.934f, 0.411f, 0.797f, 0.344f,
						0.587f, 0.341f, 0.434f, 0.375f, 0.257f,
						0.457f, 0.12f,
						0.567f, 0.075f
					});
					break;
				case ')':
					DebugDrawLines(position, color, size, duration, new[] {
						0.442f, 0.94f, 0.548f, 0.757f, 0.625f,
						0.568f, 0.64f, 0.392f, 0.554f, 0.129f,
						0.472f, 0.056f
					});
					break;
				case ';':
				case ',':
					DebugDrawLines(position, color, size, duration, new[] {
						0.533f, 0.239f, 0.527f, 0.154f, 0.487f,
						0.099f, 0.451f, 0.062f
					});
					break;
				case '_':
					DebugDrawLines(position, color, size, duration, new[] {
	 ...
	 case ':': DebugDrawLines(position, color, size, duration, new [] {
		...
		public static void DebugDrawLines(Vector3 position, Color color, float size, float duration, float[] points) {
			...
			for (var i = 0; i < points.Length; i += 2) {
				var vec = new Vector3(points[i + 0], points[i + 1]);
				vecList.Add(vec);
			}
		...
		public static void ClearLogConsole() {
			...
			//Debug.Msg("################# DISABLED BECAUSE OF BUILD!");
			/*
		...
		public static string GetPercentString(float f, bool includeSign = true)
			=> Mathf.RoundToInt(f * 100f) + (includeSign ? "%" : "");
		...
		public static bool TestChance(int chance, int chanceMax = 100)
			=> UnityEngine.Random.Range(0, chanceMax) < chance;
		...
		public static Vector2 GetGameObjectScreenPosition2D(Transform transform) {
			var position = transform.position;
			if (CameraMain != null) {
				var worldToViewportPoint = CameraMain.WorldToViewportPoint(position);
		...
		public static Vector3 GetGameObjectScreenPosition3DEyeLevel(Transform gameobjectTransform) {
			var pos         = gameobjectTransform.position;
			var modifiedPos = new Vector3(pos.x, pos.y + 1f, pos.z);
			if (CameraMain != null) return CameraMain.WorldToViewportPoint(modifiedPos);
			return Vector3.zero;
		}

		public static Vector3 GetMouseScreenPosition3D() {
			if (CameraMain != null) return CameraMain.ScreenToViewportPoint(Input.mousePosition);
			return Vector3.zero;
		}
		...
		public static Ray? GetRayAtMousePosition() {
			if (CameraMain != null) return CameraMain.ScreenPointToRay(Input.mousePosition);
			return null;
		}

		public static float GetAbsoluteDotProductDegrees(Vector3 reference, Vector3 target) {
			var dotProduct = Vector3.Dot(reference, target);
			return Mathf.Acos(dotProduct) * Mathf.Rad2Deg;
		}

		public static float GetAngleBetweenVectors(Vector3 vectorOne, Vector3 vectorTwo)
			=> Mathf.Atan2(vectorTwo.x - vectorOne.x, vectorTwo.y - vectorOne.y) * Mathf.Rad2Deg;

		public static float GetAngleBetweenVectors2D(Vector2 vectorOne, Vector2 vectorTwo)
			=> Mathf.Atan2(vectorTwo.x - vectorOne.x, vectorTwo.y - vectorOne.y) * Mathf.Rad2Deg;

		public static Vector3 RotatePointAround(Vector3 point, Vector3 around, Quaternion rotation)
			=> rotation * (point - around) + around;

		public static IEnumerator WaitForWait(int numberOfFrames) {
			var framesToWaitFor = numberOfFrames < 1 ? 1 : numberOfFrames;

			while (framesToWaitFor >= 1) {
				framesToWaitFor--;
				yield return null;
			}
		}
		...
		public static void EventInvokeTestLog(object sender, EventArgs e) {
			...
			Debug.Log($"Testing event from {sender}-----");
			Debug.Log($"EventArgs passed: {e}-----");
		}

		public static class Gizmos {
			public static void Draw2DCircle(Vector3 center, float radius, Color color, int segments = 20) {
				...
				var         position = center + new Vector3(x, y, 0);
				Vector3     newPosition;
				var         lastPosition = position;

				for (angle = step; angle < twoPI; angle += step) {
					x           = radius * Mathf.Cos(angle);
					y           = radius * Mathf.Sin(angle);
					newPosition = center + new Vector3(x, y, 0);
					UnityEngine.Gizmos.DrawLine(position, newPosition);
					position = newPosition;
				}

				UnityEngine.Gizmos.DrawLine(position, lastPosition);
			}
		}

		public static class Floats {
			public const float Power10    = 0.1f;
			public const float Power100   = 0.01f;
			public const float Power1000  = 0.001f;
			public const float Power10000 = 0.00001f;
		}
		...
		class JsonDictionary {
			...
			public List<string> valueList = new();
		}
		...
		public static class ReflectionTools {
			...
			public static object GetField(string typeName, string fieldName) {
				var fieldInfo = Type.GetType(typeName).GetField(fieldName);
				return fieldInfo.GetValue(null);
			}

			public static Type GetNestedType(string typeName, string nestedTypeName)
				=> Type.GetType(typeName).GetNestedType(nestedTypeName);
		}

		public struct DropDownListGenerator {
			const BindingFlags _bindingFlagsConstStrings = BindingFlags.Public |
			                                               BindingFlags.Static |
			                                               BindingFlags.FlattenHierarchy;

			const BindingFlags _bindingFlagsStaticStrings = BindingFlags.Public | BindingFlags.Static;
			...
			public static IEnumerable CreateListConstStrings<T>() where T : struct {
				...
				var constantFields =
					structType.GetFields(_bindingFlagsConstStrings)
			...
			public static IEnumerable CreateListStaticStrings<T>() where T : struct {
				...
				var structType   = typeof(T);
				var constantFields =
					structType.GetFields(_bindingFlagsStaticStrings)
