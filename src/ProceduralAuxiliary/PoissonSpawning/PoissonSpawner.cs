...
namespace ProceduralAuxiliary.PoissonSpawning {
	[RequireComponent(typeof(PoissonObjectHandler))]
	[ExecuteInEditMode]
	public class PoissonSpawner : MonoBehaviour {
		...
		IPoissonObjectHandler               _objectHandler;
		...
		void SetStart() {
			...
			_start = new GameObject("PoissonStart") {
				transform = {
					parent        = transform,
					localPosition = new Vector3(-size.x / 2f, -size.y / 2f, 0f)
				}
			};
		}
		...
		void RemoveLabel() {
			...
			if (_label.transform != null) {
				if (Application.isPlaying) Destroy(_label.transform.gameObject);
				else DestroyImmediate(_label.transform.gameObject);
			}
		}
		...
		public void Spawn() {
			...
			var bounds          = PoissonBounds.GetBounds(data.weightTable);
		...
		public IPoissonObjectHandler ObjectHandler => _objectHandler ??= GetComponent<PoissonObjectHandler>();
	}
}